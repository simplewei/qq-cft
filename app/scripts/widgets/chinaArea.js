...
(function (factory) {
    ...
    chinaArea.init = function (provinceSelectId, citySelectId, districtSelectId) {
        return new ChinaArea(provinceSelectId, citySelectId, districtSelectId);
    };
    //构造函数
   ...
   // adrrArr数组
   // 形似 ['安徽省', '合肥市', '庐阳区']
   ChinaArea.prototype.set = function(adrrArr){

        var areaObj = getAreaObj(adrrArr);
        if(areaObj){
            $('#province').val(areaObj.province.code+'|'+areaObj.province.name)
            this.initCities();
            $('#city').val(areaObj.city.code+'|'+areaObj.city.name)
            this.initDistricts();
            $('#district').val(areaObj.district.code+'|'+areaObj.district.name)
        }
   }

   var getAreaObj = function(adrrArr){
            
        var _prov = $(areaData).filter(function(){
            return this.name == adrrArr[0];
        });

        if(!_prov[0])
            return false;

        var _city = $(_prov[0]['city']).filter(function(){
            return this.name == adrrArr[1];
        });

        if(!_city[0])
            return false;

        var _dist = $(_city[0]['area']).filter(function(){
            return this.name == adrrArr[2];
        });

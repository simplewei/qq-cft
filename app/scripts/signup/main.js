require(['zepto', 'backbone', 'underscore', 'iscroll', 'queryString', 'widgets/wxLogin',
 ...
 'text', 'widgets/iscrollFix', 'widgets/ajaxSettings'],
		...
		wxLogin.login().then(function(data) {
			...
			var AppRouter = Backbone.Router.extend({
				...
				sign: function() {
					...
					/* 
					 * 新手指引
					 * 当用户提交订单后将localstorage标志位设为true
					 */

					(function(ls){
						if(ls && ls.getItem('guide'))
							return;
						var _offset = $('#wrapper').prev().removeClass('hide').offset();
						if(_offset)
							$('#wrapper').css('top', _offset.height-20)
					})(localStorage)
					$('header p span').removeClass('selected').eq(0).addClass('selected');
